...
add_hook('ClientAreaPage', 1, function ($params)
{
    ...
    else if(file_exists(getcwd().DS.'modules'.DS.'servers'.DS.'RealtimeRegisterSsl'.DS.'Loader.php'))
    {
        $loaderdir = getcwd().DS.'modules'.DS.'servers'.DS.'RealtimeRegisterSsl'.DS.'Loader.php';
    }
    if($loaderdir === false)
    {
        return;
    ...
    if (isset($params['templatefile']))
    {
        ...
        switch ($params['templatefile'])
        {
            case 'configureproduct':
                $product = DB::table('tblproducts')->where('id', $params['productinfo']['pid'])->where('servertype', 'RealtimeRegisterSsl')->first();
            ...
            case 'clientareaproductdetails':
                $product = DB::table('tblproducts')->where('id', $params['pid'])->where('servertype', 'RealtimeRegisterSsl')->first();
                $includedsan = $product->configoption4;
                $includedsanwildcard = $product->configoption8;
                ...
                if($includedsan > 0)
                {
                    $txt = sprintf (\MGModule\RealtimeRegisterSsl\mgLibs\Lang::getInstance()->absoluteT('additionalSingleDomainInfo'), $includedsan);
                    $txtincluded .= '<p>'.$txt.'</p>';
                }
                if($includedsanwildcard > 0)
                {
                    $txt = sprintf (\MGModule\RealtimeRegisterSsl\mgLibs\Lang::getInstance()->absoluteT('additionalSingleDomainWildcardInfo'), $includedsanwildcard);
                    $txtincluded .= '<p>'.$txt.'</p>';
                }                
                $smarty->assign('txtincluded', $txtincluded);
                $smarty->assign('customRealtimeRegisterSslAssetsURL', main\Server::I()->getAssetsURL());
...
if ($vars['filename'] == 'clientsservices' && $_GET['userid'] && $_GET['id']) {
    ...
    ->first();
    if($hosting->servertype != 'RealtimeRegisterSsl') return;
    return <<<JS
