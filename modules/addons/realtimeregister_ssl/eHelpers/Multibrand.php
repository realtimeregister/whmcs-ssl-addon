...
class Multibrand
{
    public static function getBrandData($id)
    {
        ...
        if (function_exists('MultibrandFunctionalityAutoLoader') && !empty($mb1x)) {
            ...
            $check  = BrandRelations::factory()->searchFor('service', $id);
            
            if (isset($check->brand_id)) {
                $currentBrand = Brand::factory($check->brand_id)->fetchOne();
            }            
            //if not assigned to any of the brands.
            if (empty($currentBrand)) {
                $currentBrand = Brand::factory()->getDefaultOne();
            }
            ...
            $brandData['LogoURL'] = $currentBrand->systemURL
                . '/modules/addons/MultibrandFunctionality/storage/logos/' . $currentBrand->logo;
            $brandData['LogoImage'] = '<img src="' . $currentBrand->systemURL
                . '/modules/addons/MultibrandFunctionality/storage/logos/' . $currentBrand->logo . '"/>';
            ...
            return $brandData;
        } elseif (file_exists(dirname(dirname(dirname(__DIR__)))
                . DIRECTORY_SEPARATOR . 'addons' . DIRECTORY_SEPARATOR . 'Multibrand' . DIRECTORY_SEPARATOR
                . 'Loader.php'
            ) && !empty($mb2x)) {
            ...
            $systemURL = Server::getSystemURL($settings['domain']);
            ...
            $brandData['LogoImage'] = '<img src="' . $systemURL . 'modules/addons/Multibrand/storage/logo/'
                . $settings['logo'] . '"/>';
