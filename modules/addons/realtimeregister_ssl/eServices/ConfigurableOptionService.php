...
class ConfigurableOptionService
{
    ...
    public static function createForProduct($productId, $apiProductId, $name, $apiProduct, $optionGroupId)
    {
        ...
        if ($optionGroupId == null) {
            $optionGroupId = self::createOptionGroup($name, $productId);
        }

        self::insertOptions($apiProductId, $apiProduct, $optionGroupId);
    }

    public static function createForProductWildCard($productId, $apiProductId, $name, $apiProduct, $optionGroupId)
    {
    ...
    public static function insertPeriods($productId, $apiProductId, $name, array $periods): ?int
    {
    ...
    private static function createOptionGroup($name, $productId) : int {
        $optionGroup = [
            'name' => 'RealtimeRegisterSsl - ' . $name,
            'description' => 'Auto generated by module - RealtimeRegisterSsl #' . $productId
        ];
    ...
    private static function insertOptions($apiProductId, $optionGroupId, $apiProduct) {
        ...
        foreach($periods as $i => $period) {
            $price = $priceRepo->onlyApiProductID(KeyToIdMapping::getIdByKey($apiProductId))
                    ->onlyPeriod($period)
                    ->onlyAction('EXTRA_DOMAIN')
                    ->fetchOne()
                    ->price / 100;
            $option = [
                'gid' => $optionGroupId,
                'optionname' => provisioning\ConfigOptions::OPTION_SANS_COUNT . "|Additional Single domain SANs (" . $period / 12 . 'years)',
                'optiontype' => 4,
                'qtyminimum' => 0,
                'qtymaximum' => $apiProduct->getMaxDomains() - $apiProduct->getIncludedDomains(),
