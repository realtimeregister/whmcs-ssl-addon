...
class Plesk extends Client implements PlatformInterface
{
    ...
    public function genKeyCsr(string $domain, array $csrData): array
    {
        ...
        if (
            isset($response->certificate->generate->result->status)
            && (string)$response->certificate->generate->result->status == 'error'
        ) {
    ...
    public function uploadCertificate(string $domain, array $crt): string
    {
        return "success";
    }
    ...
    public function installCertificate(
        ...
        if (
            isset($response->certificate->install->result->status)
            && (string)$response->certificate->install->result->status == 'error'
        ) {
