...
class Renew
{
    ...
    private function renewCertificate()
    {
        ...
        $addSSLRenewOrder = ApiProvider::getInstance()->getApi()->addSSLRenewOrder($this->getOrderParams());
        ...
        if (file_exists($dnsmanagerfile) && $checkTable !== false) {
            ...
            $loaderDNS = dirname(
                    dirname(dirname(dirname(dirname(__DIR__))))
                ) . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . 'addons' . DIRECTORY_SEPARATOR
                . 'DNSManager2' . DIRECTORY_SEPARATOR . 'loader.php';
            ...
            $records = [];
            if (
                isset($addSSLRenewOrder['approver_method']['dns']['record'])
                && !empty($addSSLRenewOrder['approver_method']['dns']['record'])
            ) {
    ...
    private function getOrderParams()
    {
         ...
         * otherwise use client details
         */
        $apiConf = (new Repository())->get();
        $useAdminContact = $apiConf->use_admin_contact;

        $order['tech_firstname'] = ($useAdminContact) ? $p->firstname : $apiConf->tech_firstname;
        $order['tech_lastname'] = ($useAdminContact) ? $p->lastname : $apiConf->tech_lastname;
        $order['tech_organization'] = ($useAdminContact) ? $p->orgname : $apiConf->tech_organization;
        $order['tech_addressline1'] = ($useAdminContact) ? $p->address1 : $apiConf->tech_addressline1;
        $order['tech_phone'] = ($useAdminContact) ? $p->phonenumber : $apiConf->tech_phone;
        $order['tech_title'] = ($useAdminContact) ? $p->jobtitle : $apiConf->tech_title;
        $order['tech_email'] = ($useAdminContact) ? $p->email : $apiConf->tech_email;
        $order['tech_city'] = ($useAdminContact) ? $p->city : $apiConf->tech_city;
        $order['tech_country'] = ($useAdminContact) ? $p->country : $apiConf->tech_country;
        ...
        if ($orgRequired && isset($f->org_name)) {
            ...
            if (strlen($f->org_country) != 2) {
                $order['org_country'] = Countries::getInstance()->getCountryCodeByName($f->org_country);
            } else {
