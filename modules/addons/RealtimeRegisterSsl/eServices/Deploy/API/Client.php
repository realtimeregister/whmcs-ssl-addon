...
namespace MGModule\RealtimeRegisterSsl\eServices\Deploy\API;
...
class Client
{
    ...
    private $ua = 'RealtimeRegisterSsl-%s/WHMCS-%s';
    ...
    public function url(array $parts = [], bool $reset = false, bool $rebuild = false): Client
    {
        if ($rebuild) {
            $this->url = $this->params['API_URL'];
        }
    ...
    private function setOptions(string $method, array $post, $content = null): array
    {
        $this->options = [
    ...
    protected function request(string $method = 'GET', array $post = [], string $content = null): string
    {
        ...
        throw new DeployException("Something went wrong");
    }
    ...
    protected function debug()
    {
        ...
        logModuleCall(
            "Realtime Register SSL Deploy",
            $this->method,
            print_r($this->request, true),
            implode(PHP_EOL, $this->response)
        );
    }
