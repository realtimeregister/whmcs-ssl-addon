...
class ScriptService {
    ...
    public static function getWebServerTypeSctipt($apiWebServersJSON) {
        $servertype = \MGModule\RealtimeRegisterSsl\eServices\FlashService::getFieldsMemory($_GET['cert'], 'servertype');
        if($servertype != '-1') {
    ...
    public static function getGenerateCsrModalScript($serviceId, $fillVarsJSON, $countriesForGenerateCsrForm, $vars = array()) {
        ...
        if($profile_data_csr)
        {
            ...
            $client =  new \MGModule\RealtimeRegisterSsl\models\whmcs\clients\Client($service->clientID);
