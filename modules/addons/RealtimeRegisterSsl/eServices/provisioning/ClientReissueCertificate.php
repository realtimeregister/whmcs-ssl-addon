...
class ClientReissueCertificate {
    ...
    private function miniControler() {
        ...
        $this->vars['generateCsrIntegrationCode'] =   ($displayCsrGenerator) ? \MGModule\RealtimeRegisterSsl\eServices\ScriptService::getGenerateCsrModalScript($this->p['serviceid'], json_encode(array()), $countriesForGenerateCsrForm) : '';
    ...
    private function stepOneForm() {
        ...
        $parseDomains                 = \MGModule\RealtimeRegisterSsl\eHelpers\SansDomains::parseDomains(strtolower($domains));
        $domainsWildcard              = $this->post['sans_domains_wildcard'];
        $parseDomainsWildcard         = \MGModule\RealtimeRegisterSsl\eHelpers\SansDomains::parseDomains(strtolower($domainsWildcard));
    ...
    private function stepTwoForm() {
        ...
        if ($this->getSansLimit()) {
            $this->validateSanDomains();
            $sansDomains             = \MGModule\RealtimeRegisterSsl\eHelpers\SansDomains::parseDomains($this->post['sans_domains']);
            $sansDomainsWildcard = \MGModule\RealtimeRegisterSsl\eHelpers\SansDomains::parseDomains($this->post['sans_domains_wildcard']);
    ...
    private function validateWebServer() {
        if($this->post['webservertype'] == 0) {
            throw new Exception(\MGModule\RealtimeRegisterSsl\mgLibs\Lang::getInstance()->T('mustSelectServer'));
        }
    }
    ...
    private function validateSanDomains() {
        ...
        $invalidDomains = \MGModule\RealtimeRegisterSsl\eHelpers\Domains::getInvalidDomains($sansDomains, in_array($apiProductId, self::PRODUCTS_WITH_ADDITIONAL_SAN_VALIDATION));
    ...
    private function validateService() {
        ...
        if (is_null($this->sslService)) {
            throw new Exception(\MGModule\RealtimeRegisterSsl\mgLibs\Lang::getInstance()->T('createNotInitialized'));
        }
        ...
        if ($this->sslService->configdata->ssl_status !== 'active') {
            throw new Exception(\MGModule\RealtimeRegisterSsl\mgLibs\Lang::getInstance()->T('notAllowToReissue'));
        }
    ...
    private function loadServerList() {
        ...
        try {
            ...
            \MGModule\RealtimeRegisterSsl\eServices\FlashService::set('REALTIMEREGISTERSSL_SERVER_LIST_' . \MGModule\RealtimeRegisterSsl\eServices\provisioning\ConfigOptions::API_PRODUCT_ID, $apiWebServers);
        } catch (Exception $ex) {
            $this->vars['errors'][] .= \MGModule\RealtimeRegisterSsl\mgLibs\Lang::getInstance()->T('canNotFetchWebServer');
        }
    ...
    private function build($template) {
        ...
        $content             = \MGModule\RealtimeRegisterSsl\eServices\TemplateService::buildTemplate($template, $this->vars);
        return [
            'templatefile' => 'main',
