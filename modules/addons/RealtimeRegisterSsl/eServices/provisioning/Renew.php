...
class Renew {
    ...
    public function run() {
        ...
        {
            return 'This action cannot be called, it will only be called when paying for a renew invoice. If you want to run this action manually please check the "Renew order via existing order" option in the RealtimeRegisterSsl module settings.';
        }
    ...
    private function checkRenew($serviceid)
    {
        ...
        $renew = Capsule::table('mgfw_REALTIMEREGISTERSSL_renew')->where('serviceid', $serviceid)->where('date', 'like', date('Y-m-d H').'%')->first();
    ...
    private function addRenew($serviceid)
    {
        ...
        if(isset($renew->id) && !empty($renew->id))
        {
            Capsule::table('mgfw_REALTIMEREGISTERSSL_renew')->where('serviceid', $serviceid)->update(array(
                'date' => date('Y-m-d H:i:s')
            ));
        }
        else
        {
            Capsule::table('mgfw_REALTIMEREGISTERSSL_renew')->insert(array(
                'serviceid' => $serviceid,
                'date' => date('Y-m-d H:i:s')
            ));
        }
    ...
    private function renewCertificate() {
        ...
        $addSSLRenewOrder = \MGModule\RealtimeRegisterSsl\eProviders\ApiProvider::getInstance()->getApi()->addSSLRenewOrder($this->getOrderParams());
    ...
    private function getOrderParams() {        
        ...
        if ($orgRequired && isset($f->org_name)) {
            ...
            {
                $order['org_country']      = \MGModule\RealtimeRegisterSsl\eRepository\whmcs\config\Countries::getInstance()->getCountryCodeByName($f->org_country);
            }
            else
            {
...
//            $order['org_duns']         = $f->org_duns;
            ...
            $order['org_country']      = \MGModule\RealtimeRegisterSsl\eRepository\whmcs\config\Countries::getInstance()->getCountryCodeByName($p->country);
