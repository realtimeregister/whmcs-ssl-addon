...
class Invoice
{
    ...
    const INVOICE_INFOS_TABLE_NAME = 'mgfw_REALTIMEREGISTERSSL_invoices_info';
    ...
    {
        $clientRepo = new \MGModule\RealtimeRegisterSsl\models\whmcs\clients\Client($id);
    ...
    public function createInvoice($service, $product, $returnInvoiceID = false) {
        ...
        $commission = \MGModule\RealtimeRegisterSsl\eHelpers\Commission::getCommissionValue(array('pid' => $service->packageid, 'client' => $service->userid));
        ...
        {   
            ...
            {
                $productConfiguration = Capsule::table("tblproducts")->where("tblproducts.servertype", "=", "RealtimeRegisterSsl")->where("id", "=", $service->packageid)->first();
    ...
    public function CopyConfigurationAndSendRenewToGGSSL($newOrderId)
    {
        ...
        $GenerateSCR = new \MGModule\RealtimeRegisterSsl\eServices\provisioning\GenerateCSR($post_temp, $params_temp);
...
//        }
    ...
    private function getSanCountConfigOptionServiceDetails($service)
    {
        ...
        if($isSanEnabledWildcard)
        {
            ...
            {
                \MGModule\RealtimeRegisterSsl\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl("Realtime Register Ssl WHMCS: Required Product Module '" . $server->getServiceModule() . "' Missing");
            }
            else
            {
                $serviceParams = $server->buildParams();
                if (!empty($serviceParams)) {
                    $CORepo = new \MGModule\RealtimeRegisterSsl\models\whmcs\service\configOptions\Repository($service->id);
    ...
    public function createOrder($userId, $paymentMethod, $productID, $domain, $dueDateNewOrder, $serviceBillingCycle, $configOptions = array()) {
        ...
        \MGModule\RealtimeRegisterSsl\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl(print_r($postData, true));
        \MGModule\RealtimeRegisterSsl\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl(print_r($result, true));
        return $results;
    }
    ...
    {        
        try
            {
            $service = new \MGModule\RealtimeRegisterSsl\models\whmcs\service\Service($serviceID);
            ...
            $invoiceItemsRepo = new \MGModule\RealtimeRegisterSsl\models\whmcs\invoices\RepositoryItem();
            ...
            {     
                ...
                if($newDescription)
                {
                    ...
                    \MGModule\RealtimeRegisterSsl\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl("Realtime Register Ssl WHMCS: Description of the invoice item for Invoice ID: " . $invoice->getId() . ' has been changed from "' . $oldDescription . '" to "' . $newDescription . '"');
                }
            } 
        }
        catch(\Exception $e)
