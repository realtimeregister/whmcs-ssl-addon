...
class Migration
{
    ...
    const MODULE_NAME                  = 'REALTIMEREGISTERSSLWHMCS';
    ...
    public function run()
    {
        try
        {
            main\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl('Realtime Register Ssl WHMCS: Data & Configuration Migration Started');
            ...
            main\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl('Realtime Register Ssl WHMCS: Data & Configuration Migration Completed');
        }
        catch (Exception $ex)
        {
    ...
    private function importModuleConfiguration()
    {
        if (self::$moduleLoaded)
        {
            ...
            main\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl('Realtime Register Ssl WHMCS: Module '.self::MODULE_NAME.' configuration imported successfully.');
        }
        else
        {
            main\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl('Realtime Register Ssl WHMCS: Module '.self::MODULE_NAME.' installation not detected.');
        }
    }
    ...
    private function updateExistingProducts()
    {
        ...
        foreach ($productsRepo->get() as $product)
        {
            ...
            main\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl('Realtime Register Ssl WHMCS: Product ID: ' . $product->id . ' has been modified. Module changed from ' . self::MODULE_NAME . ' to ' . main\Addon::I()->configuration()->systemName);
        }
    }
    ...
    private function updateSSLOrders()
    {
        ...
        foreach ($orders as $ssl)
        {
            ...
            main\eHelpers\Whmcs::savelogActivityRealtimeRegisterSsl('Realtime Register Ssl WHMCS: SSL Service ID: ' . $ssl->serviceid . ' has been modified. Module changed from ' . self::MODULE_NAME . ' to ' . main\Addon::I()->configuration()->systemName);
