...
//        $this->setRealtimeRegisterSslException();
    ...
    protected function call($uri, $getData = array(), $postData = array(), $forcePost = false, $isFile = false) {
        ...
        if($uri !== '/auth/' AND !$this->key) {
            throw new RealtimeRegisterException('Authorization key is required');
        }
        $url = $this->apiUrl . $uri;
        if (!empty($getData)) {
        ...
        logModuleCall('RealtimeRegisterSsl','Time: '.$info['total_time'].' '.$uri, $info['request_header'].$queryData, $data);
        ...
        if($this->lastResponse['error'] === true AND $this->apiExceptions AND $this->exceptionType === 'RealtimeRegisterSslException') {
            throw new RealtimeRegisterException($this->lastResponse['description']);
        }

        if($this->lastResponse['error'] === true AND $this->apiExceptions AND $this->exceptionType === 'RealtimeRegisterSslApiException') {
